...
**Autoria:** ASCII hoje, Tiled quando houver mais de uma fase.

> **Desvio deliberado do plano original.** A Fase 3 manteve a fonte ASCII em
> vez de migrar para `.tmj`. O `LevelDef` — que é o contrato de verdade — não
> muda: `parseLevel()` é só o adaptador da fonte, e trocá-lo depois não toca em
> nada do `LevelBuilder` nem do gameplay. Com uma fase só, o ASCII ganha em
> duas coisas que importam agora: level design revisável em diff de PR, e zero
> ferramenta externa para editar. O custo aparece quando a fase fica grande ou
> quando forem várias — e é aí que a migração se paga.

O formato Tiled previsto, quando chegar a hora:
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **0 — Architecture** ✅   | repo, Vite/TS/ESLint/Prettier/Vitest/Playwright, `core/` com fronteira aplicada por lint, event bus, tuning, save, manifesto, scale handler, debug, CI                                                              | —                                   | **concluída** — `npm run ci` verde; 167 testes unitários; 50 testes de browser                                                                                                                |
...
| **3 — Vertical Slice** ✅ | Fase 1 completa, checkpoint, mini-boss, HUD DOM, menus, áudio, FX, progressão                                                                                                                                       | 2                                   | **concluída** — a fase vai do desembarque à extração; um teste de browser derruba o boss e termina a fase de ponta a ponta. DESVIO: a autoria continua em ASCII, não em Tiled (ver §12)       |
